...
def run_recognition_mode(project_root, custom_models=None):
    ...
    if custom_models:
        ...
        static_labels = custom_models.get('static_labels_path')
        dynamic_model = custom_models.get('dynamic_model_path')
        dynamic_labels = custom_models.get('dynamic_labels_path')
    else:
        # Get default models
        defaults = get_default_models(project_root)
        static_model = defaults.get('static_model_path')
        static_labels = defaults.get('static_labels_path')
        dynamic_model = defaults.get('dynamic_model_path')
        dynamic_labels = defaults.get('dynamic_labels_path')
    
    # Convert to strings for run_app
    static_model_str = str(static_model) if static_model else None
    static_labels_str = str(static_labels) if static_labels else None
    dynamic_model_str = str(dynamic_model) if dynamic_model else None
    dynamic_labels_str = str(dynamic_labels) if dynamic_labels else None
    
    run_app(static_model_str, dynamic_model_str, static_labels_str, dynamic_labels_str)
